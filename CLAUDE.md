...
- 데스크탑: 일반 breadcrumb
- 페이지명은 넘기지 않는다. `AppHeader`가 경로로 `src/lib/nav.ts`의 `navLabel`에서 끌어온다. 여러 단계를 그려야 할 때만 `breadcrumbs`를 직접 넘긴다
...
| 항목                     | 규칙                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 페이지 골격              | `ExplainerPage`에 `title`·`intro`를 넘긴다. h1·`max-w-5xl`·인트로 클래스를 직접 쓰지 않는다. breadcrumb은 넘기지 않는다. 경로로 `src/lib/nav.ts`에서 끌어오므로 페이지 이름의 단일 출처는 거기다                                                                                                                                                                                                 |
...
| `IllustrativeDisclaimer` | 실제 위험 예측이나 투자 판단으로 오독될 여지가 큰 페이지. 실데이터 페이지는 이름이 맞지 않으므로 같은 내용을 자체 문단으로 둔다                                                                                                                                                                                                                                                                  |
| 지표 그리드              | 기본 `grid-cols-2 sm:grid-cols-3`. 모바일은 언제나 2열이다. 4열은 서로 접을 수 없는 독립 지표가 넷이거나, 넷이 한 줄로 이어지는 파생 사슬이라 그 사슬을 보이는 것이 논지일 때만                                                                                                                                                                                                                  |
...
| `useTrajectoryPlayer(frames, speedMs)` | 끝이 정해져 있고 궤적이 프레임 배열로 나오는 것. 결정론적 캐스케이드 넷                |
