...
대조 시점은 2026년 8월이다. 시세·시장 데이터처럼 시간이 지나면 변하는 값은 `### 법인 (`/corporation`)
...
- DSR 한도 40%. 은행권 기준이고 제2금융권은 50%다
- 스트레스 DSR 3단계는 2025.7부터 하한 1.5%p였고, 10·15 대책으로 2025.10.16부터 수도권·규제지역 주택담보대출은 3.0%p다. 비수도권은 더 낮다
- 주택 유상거래 취득세율(지방세법 11조 1항 8호, 1주택): 6억 이하 1%, 6억 초과 9억 이하는 `(취득가액 × 2 ÷ 3억) − 3`%, 9억 초과 3%. 지방교육세가 취득세율의 10%만큼 더 붙는다(취득세율 × 1/2 × 20%). 두 경계 모두 연속이다
...
| LTV·DSR·스트레스 가산폭    | `lib/mortgage-models.ts` REGULATION | 위 참조                                |
| 취득세율 구간              | `lib/mortgage-models.ts`            | 지방세법 11조 1항 8호                  |
