  ...
  return (
    <div className='flex flex-col gap-4'>
      <SectionIntro title='51% 공격: 합리적이라면 정직하게 채굴한다'>
        네트워크 과반 해시파워를 확보하면 이론상 이중지불 공격이 가능하다. 하지만 그 해시파워를 갖추는 비용과, 같은
        장비로 정직하게 채굴해 버는 수익, 그리고 공격이 성공했을 때 벌어질 일을 함께 따져 보자. 슬라이더로 조건을 바꿔도
      ...
      <Card
        className={cn(
          ...
          r.costToGain >= 1 ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-rose-500/40 bg-rose-500/5',
        )}
      >
          ...
          {r.costToGain >= 1 ? (
            <>
              ...
              <b className='text-emerald-600 dark:text-emerald-400'>
                {r.costToGain >= 10 ? Math.round(r.costToGain) : r.costToGain.toFixed(1)}배
              </b>
              다. 게다가 공격이 성공하면 신뢰가 무너져 BTC 가격이 폭락하고, 비트코인 채굴에만 쓰이는 수십억 달러어치
              장비와 보유 코인이 함께 휴지가 된다. <b>합리적 행위자라면 공격 대신 정직하게 채굴해 보상을 받는다.</b>{' '}
              이것이 작업증명이 경제적 유인으로 네트워크를 지키는 방식이다.
            </>
          ) : (
            <>
              이 극단적 설정에선 이득이 비용을 넘어선다. 하지만 공격이 성공하는 순간 BTC 신뢰가 붕괴해 가격이
              폭락하므로, 노린 이득 자체가 증발하고 보유 자산·장비도 함께 파괴된다. 자기 파괴적 공격은 여전히
              비합리적이다.
            </>
          )}
        </p>
