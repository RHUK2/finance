...
export function PayoffMatrix() {
  ...
  const { cells, nash, dominantStrategy, margin } = useMemo(() => payoffMatrix({ u, r, f }), [u, r, f]);
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <Card className='gap-3 p-4'>
        <div>
          ...
          <p className='text-muted-foreground mt-1 text-sm/relaxed'>
            같은 &#39;우월전략&#39; 구조라도 보상이 다르면 결과가 정반대가 된다. 죄수의 딜레마에선 배신이 우월전략이라
            둘 다 배신(1/1)으로 수렴하지만, 이는 둘 다 협력(3/3)보다 <b>모두에게 더 나쁜</b> 결과다.
          </p>
        </div>
        <PayoffGrid
          cells={PD.cells}
      ...
      <ExplainCard
        ...
        preview="같은 '우월전략' 구조인데, 낙오 페널티 f가 유인의 방향을 통째로 뒤집는다."
        body="비트코인 채택은 흔히 죄수의 딜레마처럼 보이지만, 보상 구조가 비대칭이라 결과가 다르다. 죄수의 딜레마는 우월전략(배신)이 모두를 더 나쁜 칸으로 끌고 가는 비극이지만, 채택 게임은 낙오 페널티 f가 개인과 집단의 유인을 같은 방향으로 정렬시킨다. 남들이 살 때 나만 안 사면 자산 가치 상승에서 소외돼 상대적으로 가난해진다(낙오 페널티 f). 이 페널티 때문에 관망은 열등전략이 되고, 합리적이라면 결국 채택을 택해 '모두 채택'이라는 최선의 결과로 수렴한다. 이것이 한 국가의 채택이 다른 국가의 채택을 부르는 캐스케이드의 출발점이다."
      />
    </div>
  );
}
