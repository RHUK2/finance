...
// ECDSA P-256으로 내려온다. 어느 쪽으로 돌아도 탭의 결론은 같다. 서명은
// "어떤 도구냐"가 아니라 "누구 키냐"의 문제이기 때문이다.
...
export const ATTACK_DETAIL: Record<string, { title: string; body: string }> = {
  ...
  dev: {
    title: '여기서 검증은 끝난다',
    body: '백도어가 소스 자체에 들어가 있으면 바이너리는 소스와 정확히 일치한다. 재현 가능한 빌드는 통과한다. 그게 정확히 재현 가능한 빌드가 보증하는 바이기 때문이다. 이 줄을 막는 것은 암호가 아니라 코드 리뷰와 기여자의 평판이고, 둘 다 이 페이지가 다루는 종류의 검증이 아니다.',
  },
  fake: {
    title: '검증할 원본이 없다',
