  ...
  return (
    <div className='flex flex-col gap-4'>
      <SectionIntro title='개발자를 믿지 않고도 확인하는 방법'>
        ...
        같은 결과가 나온다면, 소스에 없는 것이 바이너리에 들어 있는지를 제3자가 잡아낼 수 있다. 아래 해시는 실제로
        계산한 값이다.
      </SectionIntro>
      ...
      <ExplainCard
        ...
        preview='도구를 가르는 것은 대개 후자다'
        body={
          <>
            <p>
              재현 가능한 빌드를 지원한다는 문장은 흔하다. 여기서 다루는 네 프로젝트가 전부 그렇다. 그런데 그것은
              가능하다는 말이지 누가 실제로 해 봤다는 말이 아니다. 아무도 해 보지 않았다면 위 화면의 빌더 A·B·C는
              존재하지 않고, 남는 것은 배포자 한 줄뿐이라 앞 탭에서 멈췄던 자리로 되돌아간다.
            </p>
            <p className='mt-2'>
              그래서 실제로 갈리는 축은 독립 재현 기록이다. 서로 다른 사람이 각자 빌드해 같은 바이트를 얻었다는 사실이
              서명과 함께 공개된 곳에 회차마다 쌓이는가. 넷 중 그런 저장소를 가진 것은 하나뿐이고, 그 하나는 검증 도구가
              유효한 서명이 몇 개 이상일 때만 통과시킬지까지 사용자가 정하게 한다. 나머지 셋에서 재현은 각자 해 보고
              각자 확인하는 일로 남는다.
            </p>
            <p className='mt-2'>
