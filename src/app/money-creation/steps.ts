...
export function buildSteps(r: number): Step[] {
  ...
  return [
    ...
    },
    {
      ...
      narration:
        '정부가 적자지출 재원을 마련하려 국채를 발행하고 은행이 이를 매입한다. 은행은 정부 계좌에 예금을 적어주고 국채를 받는다. 정부예금은 통화량에 포함되지 않으므로 아직 새 돈은 없다. 정부가 빚을 졌을 뿐이다. (실제로는 정부 계좌가 중앙은행에 있고 은행은 지준으로 대금을 치르지만, 여기서는 지준이 아직 없는 상태라 은행 예금으로 단순화했다.)',
      ops: [
    ...
    },
    {
      ...
      narration: `대출금이 다시 예치되고 또 대출되는 과정이 반복된다. 지급준비율 ${pct}%에서 통화량은 본원통화의 ${mult}배까지 확장된다. 아래 슬라이더로 지급준비율을 바꿔 승수가 어떻게 달라지는지 확인해 보라. 다만 이 승수는 확장의 상한을 보여주는 교과서 모형이다. 실제로 은행 대출을 묶는 것은 지급준비율보다 자기자본 규제와 빌릴 사람의 상환능력이고, 미국의 지급준비율은 2020년 3월부터 0%다.`,
      ops: [
...
export function metricsAt(steps: Step[], stepIndex: number): Metrics {
  ...
  const sum = (lines: Line[], item: string) => lines.find((l) => l.item === item)?.amount ?? 0;
