...
export function DollarHegemonyView() {
  ...
  return (
    <>
      <AppHeader breadcrumbs={[{ label: '달러 패권' }]} />
      <PageMain>
        <div className='mx-auto flex max-w-5xl flex-col gap-4'>
          <div>
            <h1 className='text-xl font-semibold'>달러는 왜 최악인데도 대체되지 않는가</h1>
            <p className='text-muted-foreground mt-1 text-sm/relaxed'>
              ...
              {federalDebt.source} 기준) 실물 담보는 1971년에 사라졌다. 그런데 그로부터 반세기가 지나도록 체제는
              그대로고 세계 외환보유고의 {(reserveShare.value * 100).toFixed(1)}%는 여전히 달러다({reserveShare.asOf}{' '}
              {reserveShare.source} 기준). 그러니 언제 무너지느냐를 묻기 전에 왜 아직 안 무너졌느냐를 먼저 물어야 한다.
              답은 달러가 좋아서가 아니라 나가는 값이 남는 값보다 비싸기 때문이고, 그 셈은 계산할 수 있다. 네 갈래로
              나눠 직접 밀어 보자.
            </p>
          </div>

          <IllustrativeDisclaimer>
            등장하는 등급·가중치·전가 계수·금리 반응은 구조를 보여주기 위한 예시다. 실명으로 나오는 국가의 이탈 유인
            비율은 어느 기관의 추정치도 아니고, 그 나라가 무엇을 할지에 대한 예측도 아니다. 계산이 말리는데도 움직인
            사례가 이미 있다. 이 페이지는 어떤 통화나 자산도 권하지 않는다.
          </IllustrativeDisclaimer>
