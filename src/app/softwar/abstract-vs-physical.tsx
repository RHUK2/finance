  ...
  return (
    <div className='flex flex-col gap-4'>
      <SectionIntro title='추상 권력 vs 물리 권력: 약탈자는 어디를 노리는가'>
        권력에는 두 종류가 있다. <b>추상 권력</b>은 위계·신뢰·직위처럼 믿음 위에 세워져 휘두르긴 싸지만,
        약탈자(belligerent actor)가 적은 비용으로 탈취할 수 있다. <b>물리 권력</b>은 실제 와트를 소비해 부과하는
        비용이라 비싸지만, 자산 가치보다 더 많은 에너지를 쏟아야만 뺏을 수 있게 만든다. 합리적 약탈자는 탈취 이득이
        비용보다 클 때만 공격한다. (수치는 개념용 예시)
      </SectionIntro>
  ...
  return (
    <Card className='gap-3 p-4'>
      ...
      <div className='grid grid-cols-2 gap-3'>
        ...
        <Metric label='판정' value={captured ? '탈취됨' : '방어됨'} tone={captured ? 'bad' : 'good'} />
      </div>
    </Card>
  );
}
