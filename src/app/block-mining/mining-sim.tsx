  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <ExplainCard
        ...
        preview='실제 비트코인의 목표를 사람 손으로 맞추는 건 사실상 불가능하다. ASIC은 초당 수백조 번을 찍는다.'
        body={
          <>
            사람이 계산기로 손수 해시를 두드린다면 초당 {HUMAN_RATE}회가 한계라, 이 시연의 난이도 5자리(평균{' '}
            {expectedTries(5).toLocaleString('ko-KR')}번)만 해도 하루를 넘게 두드려야 한다. 그런데 실제 비트코인의
            목표는 앞자리 0이 열아홉 자리 안팎이라, 사람 손으로는 우주의 나이를 들여도 어림없다. 실제 ASIC 채굴기 한
            대는 초당 약 {ASIC_RATE.toLocaleString('ko-KR')}번을 찍고, 전 세계 채굴기를 합치면 그보다 100만 배 넘는
            시도를 10분마다 쏟아붓는다. 정답에 &#39;가까워지는&#39; 과정이 없으니(눈사태 효과), 유일한 전략은 더 많은
            해시를 더 빨리 찍는 것뿐이고, 그게 채굴이 곧 전력 소비 경쟁이 되는 이유다.
          </>
        }
      />
    </div>
