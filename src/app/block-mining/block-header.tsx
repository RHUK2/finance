  ...
  return (
    <div className='flex flex-col gap-4'>
      <SectionIntro title='블록 헤더: 80바이트짜리 요약본'>
        블록 하나에는 수천 개의 트랜잭션이 들어 있지만, 채굴자가 실제로 반복해서 해시하는 대상은 그걸 80바이트로 압축한{' '}
        <b>헤더</b>다. 이전 블록 해시로 체인에 이어 붙고, 머클 루트로 그 안의 모든 트랜잭션을 대표하고, 나머지 필드가
      ...
      <ExplainCard
        ...
        body={
          <>
            SHA-256 같은 암호 해시 함수는 입력이 한 비트만 달라져도 출력 비트의 절반 가까이가 뒤집히도록 설계돼
            있다(눈사태 효과). 그래서 위에서 nonce를 1만 바꿔도 두 해시 사이에 어떤 규칙성이나 &#39;앞으로 갈수록 목표에
            가까워지는&#39; 패턴이 전혀 보이지 않는다. 목표를 맞는 해시를 미리 계산하거나 예측할 방법이 없고, 오직 매번
            새로 찍어서 확인하는 수밖에 없다. 이 &#39;찍기 말고는 답이 없다&#39;는 성질이 다음 탭 채굴 시뮬레이션의
            전제가 된다.
          </>
        }
      />
      ...
      <ExplainCard
        ...
        body={
          <>
            nonce는 4바이트(약 43억 가지)뿐이라 빠른 채굴기는 이걸 순식간에 다 써버린다. 그러면 블록에 자기 몫으로
            들어가는 coinbase 트랜잭션(보상을 받는 트랜잭션) 안의 <b>extraNonce</b> 값을 바꾼다. 이 트랜잭션이 바뀌면 그
            트랜잭션들을 요약하는 <b>머클 루트</b>도 통째로 바뀌므로, 사실상 시도할 수 있는 조합은 사실상 무한하다.
          </>
        }
      />
    </div>
