  ...
  return (
    <div className="flex flex-col gap-4">
      <SectionIntro title="주소 타입이 수수료를 좌우한다">
        같은 송금(같은 입력·출력 개수)이라도 주소 타입에 따라 트랜잭션 크기가
        달라 수수료가 달라진다. 지갑 키 생성에서 고른
        purpose(44&apos;/49&apos;/84&apos;/86&apos;)가 결국 이 차이로 이어진다.
        입력·출력 개수를 바꿔 절감 폭을 비교해 보자.
      </SectionIntro>
            ...
            <div key={r.value} className="flex flex-col gap-1">
              ...
              <div className="bg-muted h-7 w-full overflow-hidden rounded-md">
                <div
                  ...
                  style={{ width: `${r.width}%` }}
                >
                  <span className="text-primary-foreground text-xs tabular-nums">
                    {formatSats(r.fee)}
                  </span>
                </div>
              </div>
