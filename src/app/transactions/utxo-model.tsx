  ...
  return (
    <div className="flex flex-col gap-4">
      <SectionIntro title="동전을 고른다 (UTXO 모델)">
        ...
        생긴 <b>동전(UTXO)</b>들이 들어 있다. 송금하려면 동전을 골라 통째로
        부숴야 해서, 보낼 금액보다 큰 동전을 쓰면 나머지가 <b>잔돈</b>으로 내
        지갑에 되돌아온다. 아래에서 <b>동전을 직접 클릭해</b> 골라 보자. 고른
        동전의 합이 송금액 + 수수료를 덮으면 유효한 트랜잭션이 된다.
      </SectionIntro>
      ...
      <Card className="flex flex-col gap-4 p-4">
        ...
        <div className="flex flex-col gap-1.5 border-t pt-3">
          <span className="flex items-center gap-1.5 text-sm font-medium">
            <Coins className="size-4 text-amber-600 dark:text-amber-400" />내
            지갑의 동전들 (클릭해서 고르기)
          </span>
          {WALLET.map((u) => {
      ...
      <ExplainCard
        ...
        body={
          <>
            동전은 쪼개 쓸 수 없고 통째로만 쓸 수 있다. 12만 사토시 동전으로
            8만을 보내면, 나머지는 <b>잔돈 출력</b>으로 새 주소에 되돌려
            받는다(그래서 지갑이 매번 새 주소를 만든다). 잔돈을 만들지 않으면 그
            차액이 전부 수수료로 날아가 버린다.
          </>
        }
      />
      ...
      <ExplainCard
        ...
        body={
          <>
            은행·이더리움은 <b>계좌 잔액</b>을 더하고 빼는 방식이다. 비트코인은
            현금 지갑처럼 <b>동전(UTXO) 묶음</b>이다. 지갑 잔액은 그 동전들의
            합을 화면에서 계산해 보여줄 뿐이다. 덕분에 어떤 동전이 어디서 왔는지
            추적이 쉽고, 여러 입력을 병렬로 검증할 수 있다.
          </>
        }
      />
