  ...
  return (
    <div className="flex flex-col gap-4">
      <SectionIntro title="크기가 수수료다 (vByte × sat/vB)">
        수수료는 <b>보내는 금액과 아무 상관이 없다</b>. 트랜잭션이 블록에서
        차지하는 공간, 즉 <b>vByte 크기</b>에만 매겨진다. 입력이 많을수록(동전을
        여러 개 쓸수록) 커지고, 그만큼 비싸진다. 입력·출력 개수와 수수료율을
        바꿔 보자.
      </SectionIntro>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <Field label="입력 개수 (쓸 동전 수)">
            <Input
              ...
              value={numIn}
              onChange={(e) =>
                setNumIn(Math.max(1, Number(e.target.value) || 1))
              }
            />
          </Field>
          <Field label="출력 개수 (받는 사람+잔돈)">
            <Input
              ...
              value={numOut}
              onChange={(e) =>
                setNumOut(Math.max(1, Number(e.target.value) || 1))
              }
            />
          </Field>
          <Field label="주소 타입">
      ...
      <ExplainCard
        ...
        body={
          <>
            한 블록(약 10분에 하나)에 들어갈 공간은 약 4백만 weight(≈ 1MvB)로
            한정된다. 채굴자는 <b>vByte당 수수료가 높은</b> 트랜잭션부터 담는다.
            그래서 0.001 BTC를 보내든 100 BTC를 보내든, 크기가 같으면 수수료도
            같다. 멤풀이 붐비면 sat/vB 입찰가가 올라간다.
          </>
        }
      />
