...
export function EconomyView() {
  ...
  const chartLines = useMemo(() => {
    ...
    return {
      usdkrw: [{ data: eco.usdkrw.history, color: '#22c55e' }],
      dxy: [{ data: eco.dxy.history, color: '#22c55e' }],
      ...
      treasury: [
        ...(fred?.us2y ? [{ label: '2Y', data: fred.us2y.history, color: '#22c55e' }] : []),
  ...
  const fedFundsLines = useMemo(
    () => (fred?.fedFunds ? [{ data: fred.fedFunds.history, color: '#06b6d4' }] : undefined),
    [fred],
  );

  const yieldSpreadLines = useMemo(
    () => (yieldSpread ? [{ data: yieldSpread.history, color: '#f59e0b' }] : undefined),
    [yieldSpread],
  );
