        ...
        <div className='mx-auto flex max-w-5xl flex-col gap-4'>
          <div>
            ...
            <p className='text-muted-foreground mt-1 text-sm'>
              CPI(소비재 물가)·M2(통화량)·자산가격은 서로 다른 것을 측정합니다. 예금 금리가 통화 팽창에 못 미치면,
              저축한 과거 노동의 구매력은 조용히 줄어듭니다. 어느 쪽이 얼마나 벌어지는지 데이터로 비교합니다.
            </p>
          </div>
          ...
          {!data ? (
            ...
            <Card>
              <CardContent className='text-muted-foreground text-sm'>
                {cfg.label} 데이터는 <code className='text-foreground'>{cfg.envKey}</code> 환경변수를 설정하면
                표시됩니다.
              </CardContent>
            </Card>
          ) : (
          ...
          <p className='text-muted-foreground border-t pt-4 text-xs/relaxed'>
            CPI는 통계청/BLS 정의에 따른 측정치이며, 통화 팽창·자산가격은 별개 지표입니다. 본 페이지는 특정 측정의
            오류를 단정하지 않고, 예금 금리와 통화·자산 지표 간의 격차를 보여줍니다. M2는 2021년 정의가 변경되었고, 예금
            금리는 단기 안전금리(미국: 3개월 국채) 근사이며, 자산 수익률은 배당·세금·거래비용을 제외한 가격 기준입니다.
          </p>
        </div>
      </PageMain>
    </>
