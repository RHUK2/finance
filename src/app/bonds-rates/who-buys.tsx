...
const DETAIL: Record<string, { title: string; body: string }> = {
  'foreign-cb': {
    ...
    body: '무역 흑자로 쌓인 달러를 어딘가에 두어야 하는데, 그만한 규모를 언제든 팔 수 있게 담아 둘 곳이 미국 국채 말고는 마땅치 않다. 금리가 낮아도 산다. 팔 수는 있지만 대량으로 팔면 자국 통화가 절상되고 보유고 가치가 흔들려 쉽게 움직이지 못한다.',
  },
  'central-bank': {
    title: '값을 누르려고 산다',
    body: '중앙은행은 수익을 내려고 국채를 사는 것이 아니라 장기금리를 끌어내리려고 산다. 살 돈은 어디서 오는가. 없던 지급준비금을 새로 만들어 낸다. 그래서 중앙은행의 매입은 금리를 낮추는 쪽으로 가장 확실하게 작동하고, 거꾸로 보유 국채를 줄이기 시작하면(양적긴축) 그 힘이 반대로 걸린다.',
  },
  institutions: {
