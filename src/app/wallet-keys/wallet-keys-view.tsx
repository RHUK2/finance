          ...
          <ExplainCard
            ...
            body={
              <>
                ...
                줄이는 순수 해시다. 빠르고 단방향이라 ①의 체크섬을 비롯해 채굴·주소 생성에 두루 쓴다. <b>SHA-512</b>는
                같은 SHA-2 계열이지만 출력이 512비트로 두 배다. ②의 시드 파생과 ③의 키 트리에 모두 쓰이는데, 특히
                ③에서는 그 512비트를 정확히 반으로 갈라 앞 256비트는 키 재료, 뒤 256비트는 체인코드로 쓴다. 앞 절반이
                곧바로 키가 되는 건 마스터 단계뿐이고, 자식 단계에서는 그 값(IL)을 부모 개인키에 더해야 자식 키가 된다.{' '}
                <b>HMAC</b>은 해시에 키를 끼워 넣어, 키를 아는 사람이 만들었음을 증명하거나(인증) 두 입력을 잘 섞는
                믹서로 쓴다. <b>PBKDF2</b>는 그 HMAC을 수천 번 반복해 <b>일부러 느리게</b> 만든 키 유도 함수다. 그래서
                ②에서 PBKDF2-HMAC-SHA512(시드), ③에서 HMAC-SHA512(키 트리)가 차례로 나온다.
              </>
            }
          />
        </div>
