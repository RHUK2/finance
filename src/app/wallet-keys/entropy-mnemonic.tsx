  ...
  return (
    <div className="flex flex-col gap-4">
      <SectionIntro title="엔트로피를 단어로 (BIP-39)">
        지갑의 출발점은 순수한 무작위 비트, 즉 엔트로피다. 이 비트열 뒤에
        체크섬을 붙인 다음 11비트씩 잘라, 각 조각(0~2047)을 2048개 단어장의
        단어로 바꾼다. 강도를 바꿔 단어 수가 어떻게 달라지는지, 새 엔트로피를
        뽑아 단어가 어떻게 바뀌는지 확인해 보자.
      </SectionIntro>
      ...
      <ExplainCard
        ...
        body={
          <>
            ...
            없다. 동전 한 번이 1비트(앞 1, 뒤 0)라 128비트 지갑은 동전 128번이면
            되고, 주사위는 한 번에 약 2.58비트라 256비트도 99번이면
        ...
        <Pipeline
          items={[
            ...
            { kind: "op", label: `앞 ${bd.checksum}비트만 잘라냄 (ENT ÷ 32)` },
            {
              ...
              value: (
                <NibbleHexGrid bits={hexToBits(hash).slice(0, bd.checksum)} />
              ),
              tone: "accent",
            },
          ]}
      ...
      <ExplainCard
        ...
        body={
          <>
            ...
            정하는데, 체크섬을 <code className="font-mono">ENT ÷ 32</code>로
            잡으면 총비트가 <code className="font-mono">ENT × 33/32</code>가
            되어 언제나 11의 배수가 된다(예: 128 → 132 = 11×12단어, 256 → 264 =
            11×24단어). 즉 ÷32는 <b>남는 비트 없이 단어가 딱 떨어지게</b> 만드는
            유일한 선택이고, 덤으로 엔트로피가 길수록 체크섬도 비례해 길어져
            오타 검출력이 좋아진다.
          </>
        }
      />
      ...
      <ExplainCard
        ...
        body={
          <>
            복구할 때 지갑은 입력한 단어들에서 엔트로피를 거꾸로 뽑아{" "}
            <b>SHA-256을 다시 계산</b>하고, 끝에 붙은 체크섬과 맞는지 검사한다.
            단어를 잘못 적으면 해시가 어긋나 &#39;잘못된 니모닉&#39; 오류가
            뜬다. 그래서 아무 단어 12개나 적는다고 유효한 니모닉이 되지 않는다.
            <br />
            <br />
            다만 완벽한 그물은 아니다. 12단어의 체크섬은 <b>4비트뿐</b>이라,
            잘못 적은 니모닉이 우연히 검사를 통과할 확률이 16분의 1쯤 된다. 같은
            이야기를 뒤집으면 아무 단어 12개나 적었을 때 유효한 니모닉이 될
            확률도 딱 그만큼이다. 단어 수가 늘면 체크섬도 함께 길어져 그물이
            촘촘해진다(24단어는 8비트라 256분의 1). 체크섬은 오타를 <b>대개</b>{" "}
            잡아 주는 장치지, 반드시 잡아 준다고 믿을 물건은 아니다.
          </>
        }
      />
    </div>
