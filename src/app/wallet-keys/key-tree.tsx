    ...
    <div className='flex flex-col gap-4'>
      ...
      <Panel className='grid grid-cols-2 sm:grid-cols-3'>
        <Field label='purpose (주소 타입)'>
          <Select items={PURPOSES} value={purpose} onValueChange={(v) => v && setPurpose(v)}>
        ...
        <Field label='coin type'>
          <Select items={COINS} value={coin} onValueChange={(v) => v && setCoin(v)}>
            <SelectTrigger className='w-full'>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <ExplainCard
        ...
        body={
          <>
            ...
            <br />
            <br />
            하드닝이 막는 건 <b>아래에서 위로 올라오는 역산</b>뿐이다. 반대로 부모 xprv 자체가 새면 하드닝이든 아니든 그
            아래 모든 가지가 함께 뚫린다. 파생은 부모 개인키만 있으면 어느 쪽이든 다 되기 때문이다. 그러니 하드닝은
            시드나 상위 xprv를 지켜 주는 장치가 아니라, 이미 새어 나간 하위 키의 피해가 위로 번지지 않게 가두는 장치다.
            <br />
            <br />
            ...
            주소를 얼마든지 뽑을 수 있다. 안전한 지점까지만 잠그고, 그 아래는 편의를 택한 절충이다.
          </>
        }
      />
    </div>
