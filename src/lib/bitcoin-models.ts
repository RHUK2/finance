...
export function daysSinceGenesis(dateStr: string): number {
  return Math.max(1, Math.floor((new Date(dateStr).getTime() - GENESIS_MS) / MS_PER_DAY));
}
...
export function mvrvZScore(rows: { time: string; mvrv: number; marketCap: number }[]): SeriesPoint[] {
  const caps = rows.filter((r) => isFinite(r.marketCap) && r.marketCap > 0 && r.mvrv > 0);
...
export function movingAverage(series: SeriesPoint[], window: number): SeriesPoint[] {
  if (window <= 0) return [];
...
export function rollingVolatility(series: SeriesPoint[], window: number): SeriesPoint[] {
...
export function generateModelDates(startDate: string, endDate: string, stepDays = 7): string[] {
  const dates: string[] = [];
