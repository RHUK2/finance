...
`CLAUDE.md` 아키텍처 절에 `### 설명 페이지의 모델 배치`를 추가했다. 두 관례가 공존한다는 사실과 어느 쪽이 어느 그룹의 것인지를 적고, "새 모델은 한 페이지 전용이면 `src/app/<page>/models.ts`, 여러 페이지가 공유하면 `src/lib/`"를 기준으로 세웠다. 기존 파일을 옮기지 않는다는 것도 함께 적었다.

여러 도메인이 공유하는 값을 별도 파일로 빼는 패턴(`address-types.ts`, `bcra.ts`)도 예로 남겼다.
